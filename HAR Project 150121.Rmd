...
random.samples <- sample(rownames(testing),size=10,replace=FALSE)
data.frame( Predicted=predict(rf.mod , testing[random.samples,include.field[1:52]] ) ,
            Actual=testing[random.samples,"classe"])
```
...
Below is a feature plot of 1,000 random observations from the __training__ data, for the top 7 features by importance.

```{r fig.height=10 , fig.width=10 }
library(AppliedPredictiveModeling)
transparentTheme(trans = .4)

random.samples <- sample(rownames(training),size=1000,replace=FALSE)
